...
- [ ] 🔬 **Module libéral — brique CONVERGENCE 30 % (lots 2 et 4)**, seul morceau restant. Voir `docs/module-liberal/module_liberal_conception.md`.
  ...
  - ⚠️ **Index CCAM : tarifs en retard d'une version.** `ccam_actes.json` porte `version: CCAM v83 (effet 2026-07-01)` pour les codes, mais `tarif_act4: CCAM v80 (2025)` pour les tarifs. Écart mesuré : `HHQE002` 51,31 dans l'index contre 52,03 appliqué (~1,40 € de BR), **systématique sur les 4 356 tarifs d'activité 4**. À faire : afficher la version dans la page, puis régénérer l'index. Une vérification automatique depuis la page est **impossible** (pas d'API Cnam, pas de lecture de site tiers depuis GitHub Pages).
