...
- **`Indispos.gs` à recopier dans Apps Script** (version dépôt **`2026-07-20.3`**) — comparaison de code **insensible à la casse** (sinon un code avec minuscules en colonne G échoue), action **`resetCodeMar`** (bouton 🔄, sinon « Action inconnue ») et retrait d'`applyRotationLib`. **`code.gs` également à recopier** (version **`2026-07-20.3`** : retrait du tag `ROT-LIB` et des fonctions one-shot de conversion). Le 🔍 Diagnostic signale l'écart dépôt/déployé.
